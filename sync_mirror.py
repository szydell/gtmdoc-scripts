...
def acquire_cloudflare_session(url: str, cookies_file: Path) -> dict[str, str]:
    """
    Uses Python Playwright to bypass Cloudflare and extracts
    the cookies and User-Agent for Wget.
    Returns:
        dict with 'user_agent' and 'cookies_file' keys
    """
    print(f"[*] Acquiring Cloudflare clearance for {url} using Playwright...")
    try:
        from playwright.sync_api import sync_playwright
    except ImportError:
        print("[!] Playwright missing, please 'uv add playwright'")
    ...
    with sync_playwright() as p:
        ...
        # Navigate and wait for Anti-Bot logic
        page.goto(url, wait_until="networkidle", timeout=60000)
        page.wait_for_timeout(3000) # give JS challenges a moment
        
        user_agent = page.evaluate("navigator.userAgent")
        cookies = context.cookies()
        
        found_clearance = any(c['name'] == 'cf_clearance' for c in cookies)
        if found_clearance:
            print("[+] Found cf_clearance cookie!")
        else:
            print("[-] cf_clearance cookie not found, but saving anyway.")

        with open(cookies_file, "w") as f:
            f.write("# Netscape HTTP Cookie File\n")
            f.write("# Generated by Playwright Cloudflare Bypass\n\n")
            for c in cookies:
                domain = c['domain']
    ...
    return {"user_agent": user_agent, "cookies_file": str(cookies_file)}

def run_wget_with_retry(base_cmd: list[str], url: str, work_dir: Path, dry_run: bool, max_retries: int = 3) -> None:
    cookies_file = work_dir / "cloudflare_cookies.txt"
    
    for attempt in range(1, max_retries + 1):
        if not dry_run:
            cf_session = acquire_cloudflare_session(url, cookies_file)
            user_agent = cf_session["user_agent"]
        else:
            user_agent = "Mozilla/5.0"
            cf_session = {}
        ...
        if not dry_run and cf_session.get("cookies_file"):
            cmd.append(f"--load-cookies={cf_session['cookies_file']}")
            
        cmd.append(url)
        
        print(f"\n--- Wget Attempt {attempt}/{max_retries} ---")
        try:
            run(cmd, dry_run=dry_run, ignore_codes=[])
            print("[+] Target downloaded successfully without HTTP errors.")
            break
        except subprocess.CalledProcessError as e:
            if e.returncode == 8: # Server issued an error response (e.g. 403 Forbidden)
                print(f"[!] Wget hit a 403 or other server error (code 8).")
                if attempt < max_retries:
                    print(f"[*] Sleeping 60s for Cloudflare WAF before retry...")
                    time.sleep(60)
                else:
                    print("[!] Max retries reached. Download might be incomplete.")
            else:
                print(f"[!] Wget failed with an unexpected code: {e.returncode}")
                raise
...
def mirror(args: argparse.Namespace, script_dir: Path) -> Path:
    ...
    run_wget_with_retry(wget_base, source_url, work_dir, args.dry_run)
    for seed in args.seed_url:
        run_wget_with_retry(wget_base, seed, work_dir, args.dry_run)
